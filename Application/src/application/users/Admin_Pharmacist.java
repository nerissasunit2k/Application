...
import application.Exceptions.ExceptionHandler;
import application.Interfaces.Writers;
import application.Medicines.Allergies;
import application.Medicines.BodyPain;
import application.Medicines.Cough;
import application.Medicines.Headache;

import application.Medicines.ListOfMedicine;

/**
 *
 * @author 2ndyrGroupC
 */
public class Admin_Pharmacist {

    private Writers writers = new Writers();
    private ListOfMedicine medList = new ListOfMedicine();

    public Admin_Pharmacist() {
    }
    ...
    public ListOfMedicine getMedList() {
        return medList;
    }

    public void setMedList(ListOfMedicine medList) {
    ...
    public void addMed(Scanner write) {
        ...
        if (!medList.isMedicinePresent(medicine)) {
            ...
            switch (type) {
                case 1:
                    medList.addMedicineList(new Allergies(brandname, generic, medicine, quantity, price));
                    System.out.println("added to AllergiesMedicine");
                    break;
                case 2:
                    medList.addMedicineList(new BodyPain(brandname, generic, medicine, quantity, price));
                    System.out.println("added to BodyPainMedicine");
                    break;
                case 3:
                    medList.addMedicineList(new Cough(brandname, generic, medicine, quantity, price));
                    System.out.println("added to CoughMedicine");
                    break;
                case 4:
                    medList.addMedicineList(new Headache(brandname, generic, medicine, quantity, price));
